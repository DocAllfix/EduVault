...
| D-229 | F11 Catalog public route 2026-06-02 | **Pagina /admin/catalog era admin-only, operatori non potevano sfogliare i tipi corso disponibili**. Fix commit 5629178: 2 nuovi endpoint pubblici `GET /api/catalog/public[/{slug}]` (auth required ma non admin, forza `approved_only=True` per non leak entries pending). Nuovo componente frontend `catalog-browse` (tabella + filtri search/target + Dialog dettaglio moduli, no checkbox/approve/unapprove). Nuova route `/catalog`, entry sidebar gruppo Operativo pos 2 (dopo Dashboard, prima di Nuovo Corso). Wizard legge `?course_type=<slug>` via `useSearch` + `validateSearch` per pre-fill step 1. Riuso integrale `catalog_service.list_catalog_entries`. | accettato — flusso operatore: sfoglia catalog → click "Crea" → wizard precompilato. Admin continua a vedere `/admin/catalog` con approve/bulk-approve (invariato). |
| D-230 | F11 Audio rebuild endpoint 2026-06-02 | **Corsi vecchi con `audio_manifest_path = NULL` non avevano modo di rigenerare audio dalla webapp** — solo `POST /rebuild` esisteva, ma 10x più lento (rigenera anche PPTX/PDF) e rischio rompere file già validi. Fix commit 5629178: nuovo `app/services/audio_rebuild_service.py` (incapsula blocco audio già esistente in `rebuild_service.py:100-119` + audit log immutabile), nuovo endpoint `POST /api/courses/{id}/audio/rebuild` (gate admin+reviewer, validation `status in completed/partial`, fire-and-forget sotto Semaphore(1) coerente REI-3, idempotenza via DELETE audio_tracks prima INSERT). Errori non fatali (audio_manifest_path resta NULL → UI invariata). Frontend: `api.rebuildCourseAudio(id)` helper, callout "Audio non ancora generato" + bottone "Genera audio" (Sparkles) in course-detail card Artefatti, mostrato solo se `completed/certified + !audioReady + audioTimedOut + ruolo admin/reviewer`. Click → reset polling timer/timeout flag + invalidate query → polling esistente (12 min) rileva audio_manifest_path popolato. | accettato — pattern dedicato vs full rebuild riduce 10x tempo e zero rischio collaterale su PPTX/PDF. |
| D-235 | F12 Precache PPTX (course-detail warmCache) 2026-06-02 | **Lentezza primo accesso Course Studio (13-25s download PPTX 15-25MB)**. Fix commit 2db44dc: nuovo metodo `warmCache(token)` su `usePptxPrecache` (zero polling, scarica subito se cache miss). Wirato in `course-detail/index.tsx` useEffect: quando l'utente apre la scheda corso (completed + last_rebuilt_at), il blob PPTX viene scaricato in background. Click "Apri Studio" entro pochi secondi → preview istantanea (blob già in IndexedDB). | accettato — UX hidden latency, scelta utente (raccomandato in piano). |
| D-234 | F12 Jobs notification system 2026-06-02 | **No feedback cross-page durante rebuild/audio_rebuild/generation**: utente non sapeva quando finivano. Fix commit 2db44dc: sistema unificato `JobsStore` (Zustand + localStorage), `JobsWatcher` hook globale (polling 4s, notifica push browser + toast cliccabile CTA "Apri Studio"), `JobsBadge` pill in topbar (count + popover lista progress) wired in 12 pagine. Wiring courses-wizard, studio-topbar (rebuild + audio), course-detail (audio). Notification permission richiesta best-effort `requestNotificationPermissionOnce`. Timeout safety 20 min. | accettato — pattern Stripe Dashboard, cross-tab via Notification API native. |
| D-233 | F12 Job progress REST fallback 2026-06-02 | **Progress bar generazione ferma a 0%/"Avvio…"**: WS fallisce su Railway proxy timeout/network blip → `startPolling` cadeva su `getCourse()` che non ha `progress_percent` né `current_step`. Utente segnalato schermata con barra inerte mentre pipeline era a "Generazione contenuti". Fix commit 2db44dc: nuovo endpoint REST `GET /api/jobs/{job_id}/progress` in `app/api/websocket.py` (riusa `get_job_progress`, ownership rule identica WS). Frontend `ws.ts:startPolling` accetta `jobId` opzionale: se presente usa `api.getJobProgress(jobId)` → restituisce progress_percent + current_step. Polling interval 30s → 3s (REST cheap, latenza UI migliore). | accettato — root cause: WS solo path → REST fallback completo. |
| D-232 | F12 Image patch cross-module bug 2026-06-02 | **Cambia immagine in slide M2/M3 falliva ("Aggiornamento immagine fallito") o modificava slide sbagliata**. Root cause: `slide.index` è module-relative (riparte da 0 ad ogni modulo), `studio_service.get_slide_by_idx` trovava prima slide con `index == idx` = M0; se M0 slide-idx aveva slide_type incompatibile (es. CONTENT_TEXT/QUIZ/DIAGRAM), Pydantic validation tornava 422; altrimenti modificava M0 silenziosamente (HTTP 200 ma corso errato). Fix commit 2db44dc: backend `?module_index=N` query param opzionale su PATCH `/slides/{idx}/image` + propagato a `set_slide_image/update_slide/get_slide_by_idx` con match composito `(module_index, index)`. Frontend `api.patchSlideImage(courseId, slide.index, image, slide.module_index)`. Back-compat: caller che non passa module_index = comportamento attuale (zero rotture su altri PATCH). | accettato — pattern coerente con D-228 fix, fix puntuale via query param opzionale. |
| D-231 | F11 Image library URL fix 2026-06-02 | **Thumbnail + dialog detail in `/admin/images` mostravano icone broken**: frontend chiedeva `/{file_path}` ma backend monta `assets/` a `/static/assets` (`app/main.py:90-96`). Fix commit 5629178: cambio src a `/static/${file_path}` in 2 punti (`admin/images/index.tsx:242,484`). Inconsistenza tra mount path backend e convenzione DB (file_path relativo a `assets/`) lasciata come è — modifico solo i punti frontend (mount path `/static` è convenzione comune più sicura). | accettato — fix banale, immagini ora visibili. |
| D-228 | Course Studio cross-module bug 2026-06-02 | **Cliccare slide M2 mostrava contenuto M1 con stesso indice locale**. Verificato in prod: click "M2 slide 3 — Obiettivi principali" → editor mostra "M1 slide 3 — Applicazione della prevenzione incendi" + counter 3/343 (sbagliato — dovrebbe essere ~88/343). Root cause: `StudioSlide.index` è module-relative (riparte da 0 ad ogni modulo, vedi commento `app/api/routes/courses.py:909`); il vecchio codice `slides.find(s => s.index === selectedIdx)` ritornava il PRIMO match = sempre la slide M1. **Fix commit d399796**: cambio semantica `selectedIdx` da module-relative-index a indice globale 0..N-1 nell'array `allSlides`. `selected = allSlides[selectedIdx]` (O(1) robusto), SlideRail riceve `allSlides` come prop nuova, `posGlobal = allSlides.findIndex(x => x.index===s.index && x.module_index===s.module_index)` (match composito), `onClick={() => onSelect(posGlobal)}` invece di `onSelect(slide.index)`, `isSelected={posGlobal === selectedIdx}`, goPrev/goNext usano helper `globalIndexOf(slide)`, SlideViewer `key` composta con `module_index` per forzare ricreazione `PptxCanvasRenderer` corretto. | accettato — bug grave UX risolto, fix robusto + retro-compat con filterProblematic. |
