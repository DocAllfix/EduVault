...
>
> **Ultimo aggiornamento:** 2026-05-24 — **Audit checklist FASE 5** (12 item). 11 ✅ + 1 ❌→✅ FIX (item 8 polling fallback 30s documentato in nuovo `docs/POLLING_FALLBACK.md`). Item 11 audit_log append-only verificato LIVE in Docker: 3/3 forbidden ops (DELETE/UPDATE/TRUNCATE) → "permission denied" come nexus_app + INSERT/SELECT consentiti. Item 9 ZIP audio verificato LIVE nello smoke 5.5 (1.4MB / 31 file). Nessuna modifica al codice, solo nuova doc.
> **Conteggio attuale:** 249 test mock/deterministici + 1 test live (skipped default) + 1 script E2E run in Docker + 1 smoke E2E FASE 5 parziale + audit checklist 12/12 = 315 test pytest + 1 deselected + 1 synth E2E green + 1 smoke 5.5 + 1 audit FASE 5 (item 8 fixato), 48 discrepanze REI-16, 13 risorse mancanti (1 chiusa: #R10)
