...
| F2 | Rerank 2-stadi (D2) + Edge Graph deterministico (D1) | 🔄 | 2026-05-29 | **Step 1-4 chiusi** (rerank Cohere). COHERE_API_KEY su Railway. `app/services/retrieval_v2.py` (~470 righe, mypy+ruff verdi): `autogen_module_query` (JSON 1 LLM call), `recall_hybrid` (BM25Okapi + cosine via knowledge_repo, RRF k=60, top_k=200), `rerank_chunks` (Cohere `rerank-multilingual-v3.0` con AsyncClientV2 lazy + RRF fallback graceful), `retrieve_for_module` E2E. **Smoke test 4/4 PASS contro DB prod** via TCP proxy + Voyage init: top_score 0.97+ sui moduli con corpus reale, source=rerank_cohere, latenza accettabile. **research_agent branch** `if settings.v2_features.get("rerank_enabled"): _retrieve_chunks_per_module_v2` (~100 righe) — funzione separata per non toccare il path provato dai 3 demo. Smoke test E2E 3 moduli (Concetti / Rischi DPI / Segnaletica): chunks #1 = art. 15 / allegato V / allegato XIII (semanticamente perfetti). VAA-b `source` tracciata, VAA-d `MIN_RERANK_SCORE_ALERT=0.45` sensore-non-gate, VAA-e flag default OFF. **Resta**: F2.5 graph_service (edge deterministico+gerarchico+LLM gate Jaccard≥0.15), F2.6 ingestion step POST-embed `extract_and_index_edges`, F2.7 backfill 7 normative, F2.8 1-hop traversal, F2.9 A/B prod (Antincendio L1 corpus-thin + Specifica 4h regressione). |
