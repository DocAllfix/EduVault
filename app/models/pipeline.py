...
# ═══════════════════════════════════════════════════════════════════════
# D3 — Validated narrative skeleton (vast-hopping-sketch, post-review-17).
# The skeleton is a TAXONOMIC structure of the module (which sub-topics it
# should cover), NOT an extraction from the corpus. It is generated by an LLM
# from the domain + the course's declared normative perimeter, validated by a
# human with one click, and then drives a PER-SUB-TOPIC retrieval — replacing
# the by-title retrieval that caused cross-course contamination in V2.
# ═══════════════════════════════════════════════════════════════════════


class SkeletonItem(BaseModel):
    """One ordered sub-topic of a module skeleton.

    ``retrieval_query`` is the semantic query used to retrieve chunks for THIS
    sub-topic (not the module title) — the by-sub-topic retrieval is what makes
    each slide batch work on a disjoint, on-topic slice. The 15-char floor
    mirrors the autogen-query floor in ``retrieval_v2`` so it is a real query,
    not a bare label.
    """
...
class ModuleSkeleton(BaseModel):
    """Ordered list of 6-10 sub-topics for a single module, pending human gate.

    ``approved`` flips to True only via the explicit 1-click validation gate
    (VAA-a: verify before render). ``title`` is denormalized from
    ``ModuleSpec.title`` for the review UI.
    """
    ...
    @model_validator(mode="after")
    def _normalize_ordinals(self) -> Self:
        """Force contiguous 1..N ordinals after a user reorder/edit.

        The review UI lets the operator drag-reorder, add and remove items;
        rather than reject a slightly off sequence, we renumber deterministically
        so the persisted skeleton is always 1..N in list order (same defensive
        posture as SlideContent index reassignment).
        """
        for i, item in enumerate(self.items, start=1):
            if item.ordinal != i:
                object.__setattr__(item, "ordinal", i)
        return self
